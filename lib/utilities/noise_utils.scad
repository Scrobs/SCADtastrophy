// lib/utilities/noise_utils.scad

// Permutation array for hashing
...
// Fade function for smoothing the transition
function fade(t) = t * t * t * (t * (t * 6 - 15) + 10);

// Linear interpolation function
function lerp(t, a, b) = a + t * (b - a);

// Gradient function
function grad(hash, x, y, z) = let (
    h = hash & 15,
    u = (h < 8) ? x : y,
...
function perlinNoise(px, py, pz) = let(
    ...
    BA = perm[B] + Z,
    BB = perm[B + 1] + Z
) lerp(w, lerp(v, lerp(u, grad(perm[AA] & 255, fx, fy, fz),
                              grad(perm[BA] & 255, fx - 1, fy, fz)),
                      lerp(u, grad(perm[AB] & 255, fx, fy - 1, fz),
                              grad(perm[BB] & 255, fx - 1, fy - 1, fz))),
              lerp(v, lerp(u, grad(perm[(AA + 1) & 255], fx, fy, fz - 1),
                              grad(perm[(BA + 1) & 255], fx - 1, fy, fz - 1)),
                      lerp(u, grad(perm[(AB + 1) & 255], fx, fy - 1, fz - 1),
                              grad(perm[(BB + 1) & 255], fx - 1, fy - 1, fz - 1))));
